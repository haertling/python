...
def init():
    if not os.path.exists("HFP/baggingPlots"):
        os.makedirs("HFP/baggingPlots")
    if not os.path.exists("HFP/boostingPlots"):
        os.makedirs("HFP/boostingPlots")
    if not os.path.exists("HFP/dtreePlots"):
        os.makedirs("HFP/dtreePlots")
    if not os.path.exists("HFP/knnPlots"):
        os.makedirs("HFP/knnPlots")
    if not os.path.exists("HFP/logRegPlots"):
        os.makedirs("HFP/logRegPlots")
    if not os.path.exists("HFP/mlpPlots"):
        os.makedirs("HFP/mlpPlots")

def processHFP( ):
    # read from file
    S = pd.read_csv("data/heart_failure_clinical_records_dataset.csv")
    # use z score to remove outliers
    z = np.abs(stats.zscore(S))
    S = S[((z < 3)).all(axis=1)]
    # use minmax scaling for continuous features
    scaler = preprocessing.MinMaxScaler()
    S [['age','creatinine_phosphokinase','ejection_fraction','platelets','serum_creatinine','serum_sodium','time']] = scaler.fit_transform(S[['age','creatinine_phosphokinase','ejection_fraction','platelets','serum_creatinine','serum_sodium','time']])
    # split data into X & Y
    Y = S['DEATH_EVENT']
    X = S.drop('DEATH_EVENT', axis=1)
    # filter out features based on paper written on data set
    if(1):
        X = S.drop(['age','anaemia','creatinine_phosphokinase','diabetes','high_blood_pressure','platelets','serum_sodium','sex','smoking','time'], axis=1)
    # split the data into test and training sets
    x_train, x_test, y_train, y_test = train_test_split( X, Y, test_size=.5, random_state=4 )
    return x_train, x_test, y_train, y_test

def decision_tree_driver( x_train, x_test, y_train, y_test ):
    ...
    for depth in depths:    
        ...
        for feature in features:
            ...
            confusionMatrixDriver( d_tree, x_test, y_test, "HFP/dtreePlots/ConfusionMatrix-Depth-{}-features-{}.png".format( depth, feature ), "Depth = {}, number of features ={}".format( depth, feature ) )
            precisionRecallDriver( d_tree, x_test, y_test , "HFP/dtreePlots/precisionRecall-Depth-{}-features-{}.png".format( depth, feature ), "Depth = {}, number of features ={}".format( depth, feature ) )
        ...
        plt.savefig("HFP/dtreePlots/ROC-Depth-{}.png".format(depth))

def bagging_dtree_driver( x_train, x_test, y_train, y_test ):
    depths = [1,3,5,10,20,30,40,50]
    ...
    for depth in depths:
        ...
        for bag_size in bag_sizes:
            ...
            confusionMatrixDriver( sBag, x_test, y_test,"HFP/baggingPlots/ConfusionMatrix-Depth-{}-BS-{}.png".format( depth, bag_size ), "Depth = {}, Bag Size = {}".format( depth, bag_size ) )
            precisionRecallDriver( sBag, x_test, y_test,"HFP/baggingPlots/precisionRecall-Depth-{}-BS-{}.png".format( depth, bag_size ), "Depth = {}, Bag Size = {}".format( depth, bag_size ) )
        ...
        plt.savefig("HFP/baggingPlots/ROC-Depth-{}.png".format(depth))

def boosting_dtree_driver( x_train, x_test, y_train, y_test ):
    depths = [1,2]
    ...
    for depth in depths:
        ...
        for n_estimator in n_estimators:
            ...
            confusionMatrixDriver( sBoost, x_test, y_test,"HFP/boostingPlots/ConfusionMatrix-Depth-{}-BS-{}.png".format( depth, n_estimator ), "Depth = {}, Number of estimators = {}".format( depth, n_estimator ))
            precisionRecallDriver( sBoost, x_test, y_test,"HFP/boostingPlots/precisionRecall-Depth-{}-BS-{}.png".format( depth, n_estimator ), "Depth = {}, Number of estimators = {}".format( depth, n_estimator ))
        ...
        plt.savefig("HFP/boostingPlots/ROC-Depth-{}.png".format(depth))

def knn_driver( x_train, x_test, y_train, y_test ):
    ...
    for alg in algorithms:
        for weight in weights:
            ...
            for num_neigh in num_of_neighbors:
                ...
                confusionMatrixDriver( neigh, x_test, y_test, "HFP/knnPlots/confusionMatrix-{}-{}-{}.png".format( num_neigh, alg, weight ), "alg = {}, weight = {}, neighbors = {}".format( alg, weight, num_neigh ))
                precisionRecallDriver( neigh, x_test, y_test, "HFP/knnPlots/precisionRecall-{}-{}-{}.png".format( num_neigh, alg, weight ), "alg = {}, weight = {}, neighbors = {}".format( alg, weight, num_neigh ))
            ...
            plt.savefig("HFP/knnPlots/ROC-{}-{}.png".format(alg, weight))

def logReg_driver( x_train, x_test, y_train, y_test ):
    ...
    for solver in solvers:
        ...
        for C in Cs:
            ...
            confusionMatrixDriver( model, x_test, y_test,"HFP/logRegPlots/ConfusionMatrix-Solver-{}-C-{}.png".format(solver,C), "Solver-{} C-{}".format(solver,C))
            precisionRecallDriver( model, x_test, y_test,"HFP/logRegPlots/precisionRecall-Solver-{}-C-{}.png".format(solver,C), "Solver-{} C-{}".format(solver,C))
        ...
        plt.savefig("HFP/logRegPlots/ROC-Solver-{}.png".format(solver))

def mlp_driver( x_train, x_test, y_train, y_test ):
    ...
    for activation in activations:
        ...
        for size in sizes:
            t0 = time.time()
            mlp = MLPClassifier(solver='lbfgs', alpha=1e-5, hidden_layer_sizes=size, random_state=1, activation=activation, max_iter=5000)
            model = mlp
            ...
            confusionMatrixDriver( mlp, x_test, y_test,"HFP/mlpPlots/ConfusionMatrix-{}-{}-{}.png".format(activation, size[0], size[1] ), "Activation {}, Hidden Layer {}".format(activation, size ))
            precisionRecallDriver( mlp, x_test, y_test,"HFP/mlpPlots/precisionRecall-{}-{}-{}.png".format(activation, size[0], size[1] ), "Activation {}, Hidden Layer {}".format(activation, size ))
        ...
        plt.savefig("HFP/mlpPlots/ROC-A-{}.png".format( activation))


if __name__ == '__main__':
    init()
    x_train, x_test, y_train, y_test = processHFP()
    decision_tree_driver( x_train, x_test, y_train, y_test )
    bagging_dtree_driver( x_train, x_test, y_train, y_test )
    boosting_dtree_driver( x_train, x_test, y_train, y_test )
    knn_driver( x_train, x_test, y_train, y_test )
    logReg_driver( x_train, x_test, y_train, y_test )
    mlp_driver( x_train, x_test, y_train, y_test )
